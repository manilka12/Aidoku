...
extension DownloadManagerView {
    ...
    class ViewModel: ObservableObject {
        ...
        @Published var upscaleQueueCount = 0
...
extension DownloadManagerView.ViewModel {
    ...
    // MARK: - Upscaling functionality
    
    var canUpscale: Bool {
        ModelManager.shared.getEnabledModelFileName() != nil && !downloadedManga.isEmpty
    }
    
    func upscaleAllDownloads() {
        Task {
            await DownloadUpscaleManager.shared.queueAllDownloadsForUpscaling()
        }
    }
    
    // MARK: - Upscaling status updates
    
    private func startUpscaleStatusUpdates() {
        Timer.publish(every: 2.0, on: .main, in: .common)
            .autoconnect()
            .sink { _ in
                Task {
                    await self.updateUpscaleStatus()
                }
            }
            .store(in: &cancellables)
    }
    
    private func updateUpscaleStatus() async {
        let status = await DownloadUpscaleManager.shared.getQueueStatus()
        await MainActor.run {
            isUpscaling = status.isProcessing
            upscaleQueueCount = status.queueCount
        }
    }
}
