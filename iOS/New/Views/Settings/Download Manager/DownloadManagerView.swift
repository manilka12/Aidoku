...
struct DownloadManagerView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
                if !viewModel.downloadedManga.isEmpty {
                    Menu {
                        Button(action: viewModel.upscaleAllDownloads) {
                            Label(NSLocalizedString("UPSCALE_ALL_DOWNLOADS"), systemImage: "wand.and.stars")
                        }
                        .disabled(!viewModel.canUpscale)
    ...
    private var downloadsList: some View {
        List {
            ...
            // Upscaling status section
            if viewModel.isUpscaling || viewModel.upscaleQueueCount > 0 {
                Section {
                    HStack {
                        Image(systemName: "wand.and.stars")
                            .foregroundStyle(.blue)
                        VStack(alignment: .leading, spacing: 2) {
                            Text(viewModel.isUpscaling ? NSLocalizedString("UPSCALING_IN_PROGRESS") : NSLocalizedString("UPSCALING_QUEUED"))
                                .font(.subheadline)
                                .fontWeight(.medium)
                            if viewModel.upscaleQueueCount > 0 {
                                Text(String(format: NSLocalizedString("%i_CHAPTERS_QUEUED"), viewModel.upscaleQueueCount))
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                            }
                        }
